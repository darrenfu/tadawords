...
> **Project status:** Version `0.7.25` (build `2026072301`) makes the Parent-facing Profile-erasure **Try again** action bypass durable backoff for the exact nonterminal deletion tombstones that need attention. It preserves deletion dominance, retry history, existing Profile data, PawGoo identity, and CloudKit identifiers; it never recreates deleted child data. Exact-HEAD simulator and signed-device acceptance remain separate gates. The v0.7.5 [submission pack](Docs/APP_STORE_SUBMISSION_PACK_v0.7.5.md) remains internal until it is regenerated from an accepted signed release. See the [v0.7.25 release note](Docs/Releases/v0.7.25-family-sync-erasure-retry.md), [shipping permission inventory](Docs/SYSTEM_PERMISSION_INVENTORY_v0.7.8.md), [privacy inventory](Docs/APP_STORE_PRIVACY_v0.7.4.md), [content-rights inventory](Docs/APP_STORE_CONTENT_RIGHTS.md), [data manifest](Docs/FAMILY-SYNC-DATA-MANIFEST.md), [evidence matrix](Docs/FAMILY-SYNC-ACCEPTANCE-COVERAGE.md), [follow-up log](FOLLOWUP_BUGFIXES_AND_IMPROVEMENTS.md), [acceptance checklist](MVP_ACCEPTANCE.md), and [sync ADR](Docs/ADR-0001-CROSS-DEVICE-FAMILY-SYNC.md).
...
| Swift tests | v0.7.25 committed-HEAD source, simulator, and signed-artifact evidence pending |
...
The [feature audit](QAArtifacts/FULL_FEATURE_AUDIT_2026-07-12.md) records the merged v0.2 implementation evidence. The [follow-up log](FOLLOWUP_BUGFIXES_AND_IMPROVEMENTS.md) records changes through v0.7.19, and the [V1 backlog](V1_BACKLOG.md) lists the remaining device and human acceptance work.
