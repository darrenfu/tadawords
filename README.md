...
> **Project status:** Version `0.7.31` (build `2026072405`) retains the v0.7.28 on-device crop-and-mask editor, the owner-approved App Store 1.0 distribution contract, and v0.7.30 signed physical-device coverage for the voiceprint Keychain lifecycle. It adds a Parent-triggered Profile-erasure retry that bypasses durable backoff only for exact nonterminal deletion tombstones needing attention. Public reads still expose only complete committed generations; deletion retry preserves deletion dominance, retry history, existing Profile data, PawGoo identity, CloudKit identifiers, and LocalQA data contracts, and never recreates deleted child data. Captured and edited word-sheet photos remain on device and are not persisted. The voiceprint proof uses isolated test services and never uninstalls the user's app or touches production voiceprints. Exact-HEAD simulator and signed-device acceptance remain separate gates. See the [v0.7.31 release note](Docs/Releases/v0.7.31-family-sync-erasure-retry.md), [voiceprint lifecycle record](Docs/VOICEPRINT_KEYCHAIN_LIFECYCLE_v0.7.30.md), [decision record](Docs/APP_STORE_RELEASE_DECISIONS_v0.7.27.md), v0.7.5 [submission pack](Docs/APP_STORE_SUBMISSION_PACK_v0.7.5.md), [privacy inventory](Docs/APP_STORE_PRIVACY_v0.7.4.md), [content-rights inventory](Docs/APP_STORE_CONTENT_RIGHTS.md), [data manifest](Docs/FAMILY-SYNC-DATA-MANIFEST.md), [evidence matrix](Docs/FAMILY-SYNC-ACCEPTANCE-COVERAGE.md), and [follow-up log](FOLLOWUP_BUGFIXES_AND_IMPROVEMENTS.md).
...
| Swift tests | v0.7.31 combines v0.7.30 voiceprint lifecycle, App Store decision contracts, the v0.7.28 camera OCR editor, atomic Family Sync, Profile chooser, content-rights, and Profile-erasure manual-retry coverage; the full source gate is rerun at the immutable release HEAD |
