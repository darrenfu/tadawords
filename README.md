...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/status-v0.7.37%20Faster%20Read%20Evaluation-6D48D7" alt="v0.7.37 faster Read evaluation">
</p>
...
> **Project status:** Version `0.7.37` (build `2026072411`) shortens Read-practice evaluation latency: a high-confidence target-matching partial can complete immediately, while other usable partials settle after a 450 ms stability window without waiting again for Apple's final callback. Low-confidence evidence remains fail-closed. The release retains the conservative App Store 1.0 voiceprint fallback from v0.7.32, the v0.7.34 Profile-erasure retry, the v0.7.28 on-device crop-and-mask editor, the v0.7.30 signed Keychain lifecycle proof, and the App Store 1.0 distribution contract. See the [v0.7.37 release note](Docs/Releases/v0.7.37-speech-eval-latency.md), [v0.7.34 release note](Docs/Releases/v0.7.34-family-sync-erasure-retry.md), [1.0 fallback record](Docs/VOICEPRINT_1_0_RELEASE_FALLBACK_v0.7.32.md), [voiceprint lifecycle record](Docs/VOICEPRINT_KEYCHAIN_LIFECYCLE_v0.7.30.md), [decision record](Docs/APP_STORE_RELEASE_DECISIONS_v0.7.27.md), v0.7.5 [submission pack](Docs/APP_STORE_SUBMISSION_PACK_v0.7.5.md), [privacy inventory](Docs/APP_STORE_PRIVACY_v0.7.4.md), [content-rights inventory](Docs/APP_STORE_CONTENT_RIGHTS.md), [data manifest](Docs/FAMILY-SYNC-DATA-MANIFEST.md), [evidence matrix](Docs/FAMILY-SYNC-ACCEPTANCE-COVERAGE.md), and [follow-up log](FOLLOWUP_BUGFIXES_AND_IMPROVEMENTS.md).
...
The current v0.7.37 source candidate uses Xcode 26.6. Merged v0.7.2 passed
...
./Scripts/verify-signed-app-identity.sh \
  ...
  0.7.37 2026072411 "$(git rev-parse HEAD)" com.tadawords.app.localqa
```
...
./Scripts/verify-pawgoo-development-app.py \
  ...
  0.7.37 2026072411 "$(git rev-parse HEAD)" \
...
| Version and build | v0.7.37 (`2026072411`) in source Plists and generated project settings |
| Swift tests | v0.7.37 adds focused immediate-match, low-confidence, and stable-partial Read-recognition coverage to the retained release test surface; the full source gate is rerun at the immutable release HEAD |
