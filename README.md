...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/status-v0.7.35%20Rights%20Inventory-6D48D7" alt="v0.7.35 content-rights inventory">
</p>
...
> **Project status:** Version `0.7.35` (build `2026072409`) freezes the final evidence-backed App Store content-rights inventory after the Cartesia and Pawgoo owner evidence gates closed. It includes the v0.7.34 Parent-triggered Profile-erasure retry, which bypasses durable backoff only for exact nonterminal deletion tombstones needing attention while preserving deletion dominance, retry history, existing Profile data, PawGoo identity, CloudKit identifiers, and LocalQA data contracts. It retains the conservative App Store 1.0 voiceprint fallback: enrollment and speaker matching are not shipped, while Profile deletion and proven-fresh-install cleanup can remove an existing pre-release template. The release also retains the v0.7.28 on-device crop-and-mask editor, the v0.7.30 signed Keychain lifecycle proof, and the App Store 1.0 distribution contract: Made for Kids 6–8, Free with no IAP or ads, United States only, and manual release. See the [v0.7.34 release note](Docs/Releases/v0.7.34-family-sync-erasure-retry.md), [1.0 fallback record](Docs/VOICEPRINT_1_0_RELEASE_FALLBACK_v0.7.32.md), [voiceprint lifecycle record](Docs/VOICEPRINT_KEYCHAIN_LIFECYCLE_v0.7.30.md), [decision record](Docs/APP_STORE_RELEASE_DECISIONS_v0.7.27.md), v0.7.5 [submission pack](Docs/APP_STORE_SUBMISSION_PACK_v0.7.5.md), [privacy inventory](Docs/APP_STORE_PRIVACY_v0.7.4.md), [content-rights inventory](Docs/APP_STORE_CONTENT_RIGHTS.md), [data manifest](Docs/FAMILY-SYNC-DATA-MANIFEST.md), [evidence matrix](Docs/FAMILY-SYNC-ACCEPTANCE-COVERAGE.md), and [follow-up log](FOLLOWUP_BUGFIXES_AND_IMPROVEMENTS.md).
...
The current v0.7.35 source candidate uses Xcode 26.6. Merged v0.7.2 passed
...
./Scripts/verify-signed-app-identity.sh \
  ...
  0.7.35 2026072409 "$(git rev-parse HEAD)" com.tadawords.app.localqa
```
...
./Scripts/verify-pawgoo-development-app.py \
  ...
  0.7.35 2026072409 "$(git rev-parse HEAD)" \
...
| Version and build | v0.7.35 (`2026072409`) in source Plists and generated project settings |
| Swift tests | v0.7.35 final content-rights inventory and provenance contracts plus v0.7.34 Profile-erasure manual retry, v0.7.32 voiceprint release policy, v0.7.30 lifecycle cleanup, v0.7.28 camera OCR editor, atomic Family Sync, and Profile chooser; full source gate is rerun at the immutable release HEAD |
...
The [feature audit](QAArtifacts/FULL_FEATURE_AUDIT_2026-07-12.md) records the merged v0.2 implementation evidence. The [follow-up log](FOLLOWUP_BUGFIXES_AND_IMPROVEMENTS.md) records changes through v0.7.35, and the [V1 backlog](V1_BACKLOG.md) lists the remaining device and human acceptance work.
