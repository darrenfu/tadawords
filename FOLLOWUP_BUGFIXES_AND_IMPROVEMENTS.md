...
- All 1,008 files decode as mono 44.1 kHz AAC-LC; combined audio size is 6,810,564 bytes. The teacher pack duration is 674.80 seconds and Aurora duration is 9.05 seconds.
...
- The owner rejected the first direct-TTS launch render because `da` did not rise and the connection sounded too long. The replacement is reproducibly composed from Aurora components: three `da` pitch stages measure approximately 350→394→432 Hz, three `words` stages measure approximately 261→215→162 Hz, and a 50 ms overlap removes any inter-word silence. Apple Speech still transcribes the result as `Ta-da. Words.`; exact character and naturalness remain owner device-listening QA.
