...
The audited source keeps speech, voice enrollment, OCR, handwriting, and
...
notifications used to wake reconciliation. A PawGoo teacher-audio client and
App Attest boundary exist in the current source candidate, but
`TadaWordsTeacherAudioEndpoint` is absent from both production and LocalQA
configuration, so that path is not reachable in the audited build. Enabling it
requires a new exact-build inventory of PawGoo and ElevenLabs handling.
...
| Voice setup and derived voiceprint | Six accepted short repeat-after-me samples, with retries when needed; transient PCM; derived acoustic embedding | On device only | Each capture is about 3.4 seconds with a five-second in-memory ceiling. Enrollment samples are not persisted. The final template is stored under the Profile UUID in Keychain as `WhenUnlockedThisDeviceOnly`, with synchronization explicitly false. No tracking | Parent starts setup after permission. Profile deletion explicitly deletes the template. On a proven fresh install, bootstrap clears only Tada Words' voiceprint service before any local marker or Profile is written; a reset failure blocks bootstrap and remains retryable. Existing installations preserve templates. Signed uninstall/reinstall verification remains in #28 | [`AppleVoiceprintEnrollmentService.swift`](../Sources/TadaWordsApplePlatform/AppleVoiceprintEnrollmentService.swift), [`KeychainDeviceVoiceprintRepository.swift`](../Sources/TadaWordsApplePlatform/KeychainDeviceVoiceprintRepository.swift), [`ApplicationBootstrap.swift`](../Sources/TadaWordsAppShell/ApplicationBootstrap.swift) |
...
| Teacher word audio | Approved bundled clip or validated device-local cache. A Parent preparation request contains normalized isolated word, usage, locale, optional pronunciation key, immutable contract version, one-time challenge, opaque App Attest key ID, and assertion; it contains no Profile ID, child name, recording, transcript, or learning history | Current configuration is local-only because neither plist contains `TadaWordsTeacherAudioEndpoint`. If enabled after a new audit, the request would go to the PawGoo endpoint; PawGoo would verify App Attest, enforce limits/cache, request missing audio from ElevenLabs under its server-held contract, and return MP3 bytes plus contract/checksum headers | No provider credential enters the app. App Attest key ID is device/app-install scoped and can link authorized requests at PawGoo for abuse prevention. Audio is cached under a one-way SHA-256 filename with a checksum sidecar. Runtime retention, rate-state, cache, privacy-log, provider, and deletion practices require exact deployed-service audit before enablement | Parent import prepares before committing the Pool. Child Quest planning and playback are local-only. Missing, corrupt, unauthenticated, rejected, or unavailable audio fails visibly with no alternative voice. Enabling an endpoint invalidates this inventory until #74/#81 evidence is merged | [`TadaWordsApp.swift`](../Apps/TadaWordsApp/TadaWordsApp.swift), [`TeacherWordAudioPipeline.swift`](../Sources/TadaWordsApplePlatform/TeacherWordAudioPipeline.swift), [`RemoteTeacherWordAudioProvider.swift`](../Sources/TadaWordsApplePlatform/RemoteTeacherWordAudioProvider.swift), [`AppAttestTeacherAudioAuthorizer.swift`](../Sources/TadaWordsApplePlatform/AppAttestTeacherAudioAuthorizer.swift), [`TEACHER_AUDIO_RELEASE_GATES.md`](TEACHER_AUDIO_RELEASE_GATES.md) |
...
| Voiceprint after app removal | On exact signed iPhone/iPad builds, enroll a template, uninstall, reinstall, and prove that bootstrap clears the retained Tada Words service before seeding while an in-place upgrade preserves enrollment; then update app/site copy | [#28](https://github.com/darrenfu/tadawords/issues/28) and [Apple DTS forum guidance](https://developer.apple.com/forums/thread/36442) |
