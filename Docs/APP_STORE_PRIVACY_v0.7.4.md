...
2. Resolve Keychain behavior after app removal in issue
...
| Voice setup and derived voiceprint | Six accepted short repeat-after-me samples, with retries when needed; transient PCM; derived acoustic embedding | On device only | Each capture is about 3.4 seconds with a five-second in-memory ceiling. Enrollment samples are not persisted. The final template is stored under the Profile UUID in Keychain as `WhenUnlockedThisDeviceOnly`, with synchronization explicitly false. No tracking | Parent starts setup after permission. Profile deletion explicitly deletes the template. App-uninstall retention is unresolved in #28 | [`AppleVoiceprintEnrollmentService.swift`](../Sources/TadaWordsApplePlatform/AppleVoiceprintEnrollmentService.swift), [`KeychainDeviceVoiceprintRepository.swift`](../Sources/TadaWordsApplePlatform/KeychainDeviceVoiceprintRepository.swift) |
...
| Voiceprint after app removal | Choose and test a truthful uninstall/reinstall lifecycle; update app/site copy | [#28](https://github.com/darrenfu/tadawords/issues/28) and [Apple DTS forum guidance](https://developer.apple.com/forums/thread/36442) |
