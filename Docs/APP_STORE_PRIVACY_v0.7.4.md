...
| Distribution decisions | **Resolved for 1.0 except the single Apple band mapping:** Made for Kids, product ages 3–8, Free with no IAP/ads, United States only, no pre-order, and manual release. Apple exposes only one Kids Category band, so the owner must choose `5 and under` or `6–8` before approval. #65 must enter the exact values; #26 retains submission and release authority | [decision record](APP_STORE_RELEASE_DECISIONS_v0.7.27.md), [#24](https://github.com/darrenfu/tadawords/issues/24), [#26](https://github.com/darrenfu/tadawords/issues/26) |
