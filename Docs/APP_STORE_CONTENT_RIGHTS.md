...
- Final evidence-freeze identity: `0.7.50 / 2026072602`
...
| Historical ElevenLabs teacher candidates | Seeded and exploratory Bella/Jessica Read/Write candidates were evaluated before Bella approval | Not tracked in the current source tree and never copied into the app target | The approved shipping contract, generator, inspector, owner decisions, final manifest, and exact 4,000-clip digest are retained; obsolete candidate binaries were removed under the repository-wide Issue #114 artifact cleanup | Failed exploratory candidates are not fallback voices and are not release content. |
...
| Historical QA screenshots | Temporary PNG/JPEG captures were removed under Issue #114; `QAArtifacts` now retains only two root-level audit records and no child directory | No screenshot is copied into the application target | Durable audit observations remain in the root Markdown records; deleted captures remain recoverable from Git history | Exact App Store screenshots still require release-candidate selection and human review. |
...
| ElevenLabs approved contract and reproducible generation evidence | `Docs/TEACHER_AUDIO_OWNER_DECISIONS.md`, `Docs/TEACHER_AUDIO_RELEASE_GATES.md`, `Tools/Audio/`, and the final bundled manifest |
